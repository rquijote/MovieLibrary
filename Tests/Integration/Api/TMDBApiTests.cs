...
namespace Tests.Integration.Api
{
    public class TMDBApiTests(WebApplicationFactory<Program> factory) : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        {
            int movieId = 11;
            MovieDto expected = new() { Id = movieId, Title = "Star Wars", ReleaseDate = "1977-05-25" };
        ...
        {
            int movieId = 12;
            MovieDto expected = new() { Id = movieId, Title = "Finding Nemo", ReleaseDate = "2003-05-30" };
            var response = await _client.GetAsync($"/api/movie/{movieId}");
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        {
            int movieId = 120;
            MovieDto expected = new() { Id = movieId, Title = "The Lord of the Rings: The Fellowship of the Ring", ReleaseDate = "2001-12-18" };
            var response = await _client.GetAsync($"/api/movie/{movieId}");
            response.StatusCode.Should().Be(HttpStatusCode.OK);
